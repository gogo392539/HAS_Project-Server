...
public class ST_PuzzleDisplay_phara : MonoBehaviour
{
    // 퍼즐 Prefabs
    ...
    private void Update()
    {
        if (csStartPuzzle_phara.checkStart == true)
        {
            check = true;
            csStartPuzzle_phara.checkStart = false;
            makePuzzle();
        }
    ...
    public Vector3 GetTargetLocation(ST_PuzzleTile_phara thisTile)
    {
    ...
    private ST_PuzzleTile_phara CheckMoveLeft(int Xpos, int Ypos, ST_PuzzleTile_phara thisTile)
    {
        // 왼쪽 체크
        if ((Xpos - 1) >= 0)
        {
    ...
    private ST_PuzzleTile_phara CheckMoveRight(int Xpos, int Ypos, ST_PuzzleTile_phara thisTile)
    {
        if ((Xpos + 1) < Width)
        {
            return GetTileAtThisGridLocation(Xpos + 1, Ypos, thisTile);
    ...
    private ST_PuzzleTile_phara CheckMoveDown(int Xpos, int Ypos, ST_PuzzleTile_phara thisTile)
    {
        if ((Ypos - 1) >= 0)
        {
    ...
    private ST_PuzzleTile_phara CheckMoveUp(int Xpos, int Ypos, ST_PuzzleTile_phara thisTile)
    {
        if ((Ypos + 1) < Height)
        {
            return GetTileAtThisGridLocation(Xpos, Ypos + 1, thisTile);
    ...
    private ST_PuzzleTile_phara CheckIfWeCanMove(int Xpos, int Ypos, ST_PuzzleTile_phara thisTile)
    {
        // 움직일 방향 설정
    ...
    private ST_PuzzleTile_phara GetTileAtThisGridLocation(int x, int y, ST_PuzzleTile_phara thisTile)
    {
        ...
        for (int j = Height - 1; j >= 0; j--)
        {
            for (int i = 0; i < Width; i++)
            {
                ...
                if ((TileDisplayArray[i, j].GetComponent<ST_PuzzleTile_phara>().GridLocation.x == x) &&
                   (TileDisplayArray[i, j].GetComponent<ST_PuzzleTile_phara>().GridLocation.y == y))
                {
                    // 비활성화 시킨 타일을 발견하면 해당 타일을 반환
                    if (TileDisplayArray[i, j].GetComponent<ST_PuzzleTile_phara>().Active == false)
                    {
                        return TileDisplayArray[i, j].GetComponent<ST_PuzzleTile_phara>();
                    }
                }
            }
        }
    ...
    private IEnumerator JugglePuzzle()
    {
        ...
        //   (0,1) (1,1) (2,1)
        //   (0,0) (1,0) (2,0) }
        TileDisplayArray[0, 0].GetComponent<ST_PuzzleTile_phara>().Active = false;
        ...
        for (int k = 0; k < 2; k++)
        {
            for (int j = 0; j < Height; j++)
            {
                for (int i = 0; i < Width; i++)
                {
                    TileDisplayArray[i, j].GetComponent<ST_PuzzleTile_phara>().ExecuteAdditionalMove();
        ...
        while (Complete == false)
        {
            ...
            for (int j = Height - 1; j >= 0; j--)
            {
                for (int i = 0; i < Width; i++)
                {
                    ...
                    if (TileDisplayArray[i, j].GetComponent<ST_PuzzleTile_phara>().CorrectLocation == false)
                    {
                        Complete = false;
                    }
                }
    ...
    public void completebutton()
    {
        if (check)
        {
            ...
            check = false;
            csStartPuzzle_phara.checkComplete = true;
            Destroy(puzzle);
        }
    }
    ...
    public void gobackbutton()
    {
        if (check)
        {
            ...
            csStartPuzzle_phara.checkGoback = true;
            check = false;
        }
    }
}
