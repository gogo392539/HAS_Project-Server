...
namespace Client
{
    ...
    public class TCPClient
    {
        ...
        public void TCPServerInit()
        {
            ...
            //time out 설정을 위한 beginconnect
            TCPclient = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
            IAsyncResult result = TCPclient.BeginConnect(csMain.IPADDRESS.GO_SERVER, 36872, null, null);
        ...
        public void recvGameWait()
        {
            ...
            while (true)
            {
                ...
                {
                    //Start
                    byte[] sendData1 = new byte[csMain.MAXCOUNT.MAX_ROOMPACKET];
                    roomPacket startPacket;
                    startPacket.flag = 4;
                    startPacket.id = myID;
                    sendData1 = byteAndStruct.StructureToByte(startPacket);
                ...
                {
                    //Room Master Exit
                    byte[] sendData2 = new byte[csMain.MAXCOUNT.MAX_ROOMPACKET];
                    roomPacket RMExitPacket;
                    RMExitPacket.flag = 2;
                    RMExitPacket.id = myID;
                    sendData2 = byteAndStruct.StructureToByte(RMExitPacket);
                    TCPclient.Send(sendData2);
                    roomMasterExitSet = 1;
                    break;
                }
    ...
    public class csNetworkManager : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            if (TCPclient.getcConnectState() == -1)
            {
