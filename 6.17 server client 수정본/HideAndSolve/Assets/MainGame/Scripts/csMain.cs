...
public class csMain : MonoBehaviour
{
    ...
    public static string taggerName;    // 술래의 이름
        ...
        if (csNetworkManager.TCPclient != null)
        {
            for (int i = 0; i < MAXCOUNT.MAX_CLIENT; i++)
            {
                ...
                if (i == csNetworkManager.TCPclient.getMyID())
                {
                    ...
                    if (i != csNetworkManager.TCPclient.getTaggerSet())
                        Destroy(userArray[i].GetComponent<csCatch>());
            ...
            taggerName = userArray[csNetworkManager.TCPclient.getTaggerSet()].gameObject.name;

            userArray[csNetworkManager.TCPclient.getTaggerSet()].AddComponent<csCatch>();
